...
function createFirework(mouseX, mouseY) {
    ...
    let targetX = mouseX;
    let targetY = mouseY;
    let timeToTarget = 50 + Math.random() * 150; // between 50 and 200 frames
...
function createFireworkTail(firework) {
    let tailParticles = [];
    const tailLength = 15; // Number of segments in the tail
...
let fireworkFrequency = 0.01; // probability of creating a firework each frame

let frequencySlider = /** @type {HTMLInputElement} */ (document.createElement("input"));
frequencySlider.type = "range";
frequencySlider.min = "0";
frequencySlider.max = "0.1";
frequencySlider.step = "0.001";
frequencySlider.value = fireworkFrequency.toString();
frequencySlider.style.position = "absolute";
frequencySlider.style.top = "410px";
frequencySlider.style.left = "10px";
