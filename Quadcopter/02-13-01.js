...
const controlledDroneAnchor = { x: 300, y: 300 };
...
function loop(timestamp) {
    const deltaTime = timestamp - lastTime; // Calculate time difference since last frame
    const deltaSeconds = deltaTime / 1000;
    lastTime = timestamp; // Update lastTime for the next frame
    elapsedTime += deltaSeconds; // Keep total time so motion is continuous across frames

    context.clearRect(0, 0, canvas.width, canvas.height); // Clear the canvas for the new frame
    drawBackground(context); // Draw a simple background scene
    drawControlTower(context, elapsedTime);
    updateProjectiles(deltaSeconds);

    // draw first quadcopter
    context.save();
        context.translate(500, 150); // Center the coordinate system
        if (!quadcopters[0]) 
            generateQuadcopters(30, 0.5, 50);
        let montionFunc = flightPatterns[quadcopters[0].motionIndex];
        montionFunc(context, elapsedTime, quadcopters[0], deltaSeconds); // Call the motion function for the first quadcopter
    context.restore();

    // draw second quadcopter
    context.save();
        context.translate(120, 120); // Center the coordinate system
        if (!quadcopters[1]) 
            generateQuadcopters(60, 0.3, 80);
        montionFunc = flightPatterns[quadcopters[1].motionIndex];
        montionFunc(context, elapsedTime, quadcopters[1], deltaSeconds); // Call the motion function for the second quadcopter
    context.restore();

    // draw third quadcopter
    context.save();
        context.translate(400, 500); // Center the coordinate system
        if (!quadcopters[2])
            generateQuadcopters(45, 0.7, 65);
        montionFunc = flightPatterns[quadcopters[2].motionIndex];
        montionFunc(context, elapsedTime, quadcopters[2], deltaSeconds); // Call the motion function for the third quadcopter
    context.restore();

    // draw fourth quadcopter, which is controlled by WASD keys
    context.save();
        context.translate(controlledDroneAnchor.x, controlledDroneAnchor.y); // Center the coordinate system
        if (!quadcopters[3])
            generateQuadcopters(50, 0.5, 70);
        montionFunc = flightPatterns[quadcopters[3].motionIndex];
        montionFunc(context, elapsedTime, quadcopters[3], deltaSeconds); // Call the motion function for the fourth quadcopter
    context.restore();
...
function drawBackground(ctx) {
    ...
    drawCloud(ctx, 210, 110, 1);
    drawCloud(ctx, 470, 140, 0.8);
    drawCloud(ctx, 350, 380, 1.2);
    drawCloud(ctx, 150, 500, 0.9);
}
